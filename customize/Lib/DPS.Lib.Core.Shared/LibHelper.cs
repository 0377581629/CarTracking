...
namespace DPS.Lib.Core.Shared
{
    public static class LibHelper
    {
        ...
        {
            var salaryComponentTypeScope = (LibEnums.PageLayout)status;
        ...
        {
            return (from status in (LibEnums.PageLayout[])Enum.GetValues(typeof(LibEnums.PageLayout))
        ...
        {
            var salaryComponentTypeScope = (LibEnums.WidgetContentType)status;
            return lang.GetString(salaryComponentTypeScope.GetStringValue());
        }

        public static List<SelectListItem> ListWidgetContentType(int currentLayout, ILocalizationSource lang)
        {
            return (from status in (LibEnums.WidgetContentType[])Enum.GetValues(typeof(LibEnums.WidgetContentType))
                select new SelectListItem(lang.GetString(status.GetStringValue()), ((int)status).ToString(), currentLayout == (int)status)).ToList();
        }
