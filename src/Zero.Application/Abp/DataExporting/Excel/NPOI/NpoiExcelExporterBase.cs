...
namespace Zero.DataExporting.Excel.NPOI
{
    public abstract class NpoiExcelExporterBase : ZeroServiceBase, ITransientDependency
    {
        ...
        protected void AddObjects<T>(ISheet sheet, IList<T> items, IList<string> propertiesName)
        {
            if (items.IsNullOrEmpty() || propertiesName.IsNullOrEmpty())
            {
                return;
            }
            ...
            for (var i = 1; i <= items.Count; i++)
            {
                ...
                for (var j = 0; j < propertiesName.Count; j++)
                {
                    ...
                    if (value != null)
                    {
                        cell.SetCellValue(value.ToString()?.Replace("[", "").Replace("]", ""));
                    }
                }
            }
        }
